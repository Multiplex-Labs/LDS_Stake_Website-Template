...
@pytest.fixture(name="temple_config")
def temple_config_fixture(db_session: Session) -> "TempleRecommendConfig":
    """Ensure a TempleRecommendConfig exists with safe test defaults (cutoff=0)."""
    return ensure_temple_config(db_session)


@pytest.fixture(name="appointment_type")
def appointment_type_fixture(db_session: Session) -> "AppointmentType":
    """Return a standard test AppointmentType."""
    return make_appointment_type(db_session, name="Fixture Interview Type")


@pytest.fixture(name="interviewer_user")
def interviewer_user_fixture(db_session: Session, userpass) -> "User":
    """A regular user wired up as an interviewer (MANAGE_APPOINTMENTS via calling)."""
    user, _ = userpass
    make_interviewer_with_calling(db_session, user, prefix="Fixture")
    return user


@pytest.fixture(name="admin_token")
def admin_token_fixture(client: TestClient, admin) -> str:
    """Return a valid admin JWT access token."""
    admin_user, admin_password = admin
    return login(client, admin_user.email, admin_password)


@pytest.fixture(name="admin_auth_headers")
def admin_auth_headers_fixture(admin_token: str) -> dict:
    """Return Bearer auth headers for the admin user."""
    return auth_headers(admin_token)


@pytest.fixture(name="user_token")
def user_token_fixture(client: TestClient, userpass) -> str:
    """Return a valid JWT access token for a regular (no-permission) user."""
    user, password = userpass
    return login(client, user.email, password)


@pytest.fixture(name="user_auth_headers")
def user_auth_headers_fixture(user_token: str) -> dict:
    """Return Bearer auth headers for the regular user (no special permissions)."""
