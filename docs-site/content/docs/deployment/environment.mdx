...
|---|---|---|---|
| `DOMAIN` | Required | — | Your stake's domain (e.g., `yourstake.org`). Caddy uses this to route traffic and provision Let's Encrypt TLS for `DOMAIN`, `api.DOMAIN`, and `docs.DOMAIN`. DNS A records for all three must point to your server before starting the stack. |
