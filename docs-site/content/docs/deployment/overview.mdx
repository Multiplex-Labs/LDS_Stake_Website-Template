...
<Callout type="info">
  DNS must resolve to your server before you start the stack for the first time. Caddy provisions Let's Encrypt TLS via HTTP-01 ACME challenge, which requires port 80 to be publicly reachable. Cloudflare proxy is supported — see the [Docker Deployment](/deployment/docker) guide for both DNS paths and the Cloudflare SSL/TLS mode requirement.
</Callout>
