...
Configure DNS before running `docker compose up`. How you set up DNS determines how Caddy handles TLS — pick the path that matches your setup.

---

### Option A: Direct DNS (no proxy)

Point all three hostnames directly at your server's public IP. Caddy provisions Let's Encrypt TLS automatically using the HTTP-01 challenge, which requires port 80 to be publicly reachable.
...
<Callout type="warn">
  All three records must exist and resolve to your server before you start the stack. If any record is missing or has not propagated, Caddy fails to provision a certificate for that hostname and enters a backoff that can block the other hostnames too.
</Callout>

---

### Option B: Cloudflare Proxy (wildcard CNAME)

If you are using Cloudflare with the orange-cloud proxy enabled, use one A record and a wildcard CNAME instead of three A records:
...
The wildcard CNAME covers `api.yourstake.org`, `docs.yourstake.org`, and any other subdomain automatically.

<Callout type="warn">
  **Caddy's automatic Let's Encrypt provisioning does not work when Cloudflare proxy is enabled.** Cloudflare intercepts port 80 and redirects HTTP → HTTPS before the ACME HTTP-01 challenge reaches your server. You must use a different TLS configuration — see below.
</Callout>

**With Cloudflare proxy, replace your `Caddyfile` with the following:**
...
docs.{$DOMAIN} {
    tls internal
    reverse_proxy docs-site:3400
}
```

`tls internal` tells Caddy to generate a self-signed certificate instead of contacting Let's Encrypt. Cloudflare terminates public-facing TLS; `tls internal` just satisfies the Cloudflare → origin HTTPS leg.

**Required Cloudflare SSL/TLS setting:** Set your domain's SSL/TLS mode to **Full** (not "Flexible", not "Full (Strict)"):

- Go to Cloudflare dashboard → your domain → SSL/TLS → Overview
- Select **Full**

Flexible mode sends HTTP to your origin (insecure); Full (Strict) requires a CA-trusted cert at origin (which `tls internal` is not). **Full** accepts the self-signed cert and encrypts the Cloudflare → server leg.
...
- **Cloudflare proxy is enabled** — if you are behind Cloudflare's orange-cloud proxy, HTTP-01 ACME challenges cannot reach your server (Cloudflare intercepts port 80). Use the Cloudflare-specific Caddyfile with `tls internal` described in [Option B: Cloudflare Proxy](#option-b-cloudflare-proxy-wildcard-cname) above.
