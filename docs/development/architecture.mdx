...
<Warning>
  The frontend and slackbot share the Python `uv` tool, but their `pyproject.toml` files and virtual environments are completely separate. A `uv sync` in `backend/` does not affect `slackbot/` and vice versa.
</Warning>
